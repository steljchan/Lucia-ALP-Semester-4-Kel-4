...
export default function ScoreScreen() {
  ...
  useEffect(() => {
    const saveResult = async () => {
      if (rewardSaved || awardingRef.current) {
        return;
      }
      ...
      try {
        ...
        setCoinEarned(result.earnedCoin);
        ...
        await addDoc(collection(db, 'quizResults'), {
          ...
          skipped: skippedNum,
          total: totalNum,
          stars: result.bestStars,
          xp: result.earnedXp,
          coin: result.earnedCoin,
          replay: !result.firstCompletion,
    ...
    <View style={styles.root}>
      ...
      <LinearGradient colors={['#FFFFFF', '#C4E8FF']} style={styles.header}>
        <View style={styles.headerRow}>
          ...
          <TouchableOpacity
            onPress={() => router.push('/siswa/tabs/leaderboard')}
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.card}>
          <View style={styles.cardContent}>
            ...
            <View style={styles.xpContainer}>
              <Text style={styles.xpLabel}>{isReplay ? 'Replay Reward' : 'Reward Diperoleh'}</Text>
              ...
              <View style={styles.rewardRow}>
                ...
                <View style={styles.xpBadge}>
                  ...
                  <Text style={styles.xpBadgeText}>+{coinEarned}</Text>
                </View>

                <View style={styles.xpBadge}>
                  <Ionicons name="star" size={18} color="#F59E0B" />
                  <Text style={styles.xpBadgeText}>{earnedStars}</Text>
                </View>
              </View>

              {isReplay && (
                <Text style={{ marginTop: 8, fontSize: 12, color: COLORS.textSub }}>
        ...
        {/* ================= Bagian Pembahasan dari Kode Kedua ================= */}
            ...
            return (
              <View key={index} style={styles.quizCard}>
                ...
                {item.options.map((option: string, i: number) => {
                  ...
                  let rightIcon = null;
                  if (isUserCorrect) {
                    rightIcon = <Ionicons name="checkmark-circle" size={22} color={COLORS.success} />;
                  } else if (isUserWrong) {
                    rightIcon = <Ionicons name="close-circle" size={22} color={COLORS.error} />;
                  } else if (!isSkipped && !isUserAnswer && isCorrectAnswer) {
                    rightIcon = <Ionicons name="checkmark-circle" size={22} color={COLORS.success} />;
                  } else if (isSkipped && isCorrectAnswer) {
                    rightIcon = <Ionicons name="checkmark-circle" size={22} color={COLORS.yellow} />;
                  }
