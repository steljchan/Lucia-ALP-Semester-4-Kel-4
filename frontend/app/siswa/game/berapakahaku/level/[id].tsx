...
import {berapakahAkuLevels, formatRupiah} from '../../../../../src/data/berapakahaku';
import {saveGameProgress} from '../../../../../src/services/gameProgress';
import useBerapakahAku from '../../../../../src/hooks/useBerapakahAku';
...
export default function BerapakahAkuLevel() {
  ...
  const { selectedAnswers, select, reset, check } = useBerapakahAku(level.questions.length);
  ...
  const isAnswered = Boolean(selectedAnswers[currentQuestionIndex]);
  ...
  const handleAnswer = (option: number) => {
    ...
    select(currentQuestionIndex, option);
  };
  ...
  const goNextLevel = () => {
    ...
    if (next <= berapakahAkuLevels.length) {
      router.push(`/siswa/game/berapakahaku/level/${next}`);
    } else {
      router.back();
    }
  };
  ...
  const onSubmit = async () => {
    ...
    const isGameOver = !isCorrect && updatedHeart <= 0;
    ...
    const answers = level.questions.map((q) => q.answer);
    const resultData = check(answers);
    const rewards = calculateGameRewards({
      correctAnswers: resultData.correctCount,
      wrongAnswers: resultData.wrongCount,
    ...
    <>
      ...
      >
        <View style={styles.content}>
          ...
          >
            <Text style={styles.questionMini}>🧮 Hitung Harga</Text>
            <Text style={styles.questionIndicator}>
              Soal {currentQuestionIndex + 1} / {level.questions.length}
            </Text>
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      showAnswerResult && isSelected && answerStatus === 'correct' && styles.correctCard,
                      showAnswerResult && isSelected && answerStatus === 'wrong' && styles.wrongCard,
                    ]}
                  ...
                  >
                    ...
                    <Text style={styles.optionText}>{formatRupiah(option)}</Text>
                  </TouchableOpacity>
                );
              })}
            </View>
