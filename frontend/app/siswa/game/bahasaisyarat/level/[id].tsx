...
export default function BahasaIsyaratLevel() {
  ...
  const handleSelect = (letter: string, index: number) => {
    ...
    if (selected.length >= level.word.length) return;
  ...
  const onSubmit = async () => {
    ...
    const rewards = calculateGameRewards({
      ...
      totalQuestions: level.word.length,
    ...
    <>
      <GameLayout
        ...
        actions={[
          ...
          {
            text: isSubmitting ? 'Loading...' : 'Jawab',
            color: '#5CBEFA',
            onPress: onSubmit,
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          >
            <View style={styles.answerContainer}>
              <Text style={styles.questionWord}>{level.word}</Text>
            </View>
          </Animated.View>
            ...
            {rows.map((row, rowIndex) => (
              <View key={rowIndex} style={styles.row}>
                {row.map((letter, i) => {
                  ...
                  const selectedIndex = usedIndexes.findIndex((used) => used === index);
                  const currentStatus =
                    hasSubmitted && selectedIndex !== -1 ? letterResults[selectedIndex] : undefined;
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.signOption,
                        isSelected && !hasSubmitted && styles.signSelected,
                        hasSubmitted && currentStatus === 'correct' && styles.correctOption,
                        hasSubmitted && currentStatus === 'wrong' && styles.wrongOption,
                      ]}
                      ...
                      disabled={
                        ...
                        isSelecting ||
                        isSubmitting ||
                        selected.length >= level.word.length
                      }
                    >
                      <Image
                        source={bahasaIsyaratImages[letter.toLowerCase() as keyof typeof bahasaIsyaratImages]}
