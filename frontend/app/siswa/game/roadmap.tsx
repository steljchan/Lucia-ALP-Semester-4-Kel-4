...
export default function Roadmap({
  ...
  useEffect(() => {
    setTimeout(() => {
      ...
      if (currentIndex >= 0) {
        const reversedIndex = LEVEL_COUNT - 1 - currentIndex;
        const targetY = reversedIndex * spacingY;
  ...
  return (
    <View style={styles.container}>
      <GameHeader
        ...
        heart={heart}
        coin={coin}
        onBack={() => router.replace('/siswa/tabs/game')}
      />
      ...
      <HeartEmptyModal visible={showHeartModal} onClose={() => setShowHeartModal(false)} />
    </View>
  );
}
